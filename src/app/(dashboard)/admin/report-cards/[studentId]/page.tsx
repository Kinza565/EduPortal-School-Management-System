...
export default function ReportCardViewPage({
  ...
  useEffect(() => {
    (async () => {
      ...
      setStudentId(sid);
      setExamId(params.exam);
    })();
  }, [searchParams]);

  useEffect(() => {
    if (!studentId || !examId || !profile?.school_id) return;
    ...
    (async () => {
      ...
      try {
        ...
        const { data: studentData } = await supabase
          .from("students")
          .select("id, full_name, student_id, roll_number, father_name, guardian_name, date_of_birth, gender, photo_url, class_id, section_id")
          .eq("id", studentId)
        ...
        const results: SubjectResult[] = resultsData?.map((r) => {
  ...
  }, [studentId, examId, profile?.school_id]);
